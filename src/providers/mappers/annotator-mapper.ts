...
export const annotationsAttributeMapper = (data: any, polygons: Polygon[], pictureId: string, annotationId: string) => { // mettre ici le vrai type depuis notre client
  const { userId } = getCached.userInfo();

  return Object.entries<any>(data).map(([index, attributes]) => { // * <AreaPictureAnnotationInstanceMetadata>
