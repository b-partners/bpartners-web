...
const getProvider = (resourceType: string): HaDataProviderType => {
  ...
  throw new Error('Unexpected resourceType: '.concat(resourceType));
};

const toEnabledUsers = (users: Array<any>): Array<any> => {
  const enabledUsers: Array<any> = [];
  users.map((user) => {
...
const dataProvider: RaDataProviderType = {
  async getList(resourceType: string, params: any) {
    ...
    if (perPage > maxPageSize) {
      console.warn(
        'Page size is too big, truncating to maxPageSize='
        + maxPageSize.toString()
        + ': resourceType='
        + resourceType
        + ', requested pageSize='
        + perPage,
