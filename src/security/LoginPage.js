...
const BpLoginPage = () => {
  ...
  const onRegistration = () => {
    const initiateOnboarding = async () => {
      const { data: { redirectionUrl } } = await onboardingApi().initiateOnboarding({ redirectionStatusUrls: loginRedirectionUrls })
      redirect(redirectionUrl)
    }
    initiateOnboarding()
  }
    ...
    <Box sx={FLEX_CENTER}>
      ...
      <Box sx={{ ...FLEX_CENTER, flexShrink: 0, flexGrow: 1 }}>
        <Box sx={LOGIN_FORM}>
          ...
          }} onClick={onRegistration}>Pas de compte ? C'est par ici</Button>
        </Box>
      </Box>
      <Box width={{ md: '60%', sm: '0%', xs: '0%' }}>
