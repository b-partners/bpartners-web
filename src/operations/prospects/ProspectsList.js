...
import ChangeStautsModal from "./components/ChangeStatusModal";
...
const ProspectItem = ({prospect}) => {
    ...
    const {handleLoading, closeModal, openModal, modal:{isOpen}, setSelectedStatus} = useProspectContext();
            ...
            <form onSubmit={handleSubmit(saveOrUpdateProspectSubmit)}
                ...
                <Paper elevation={2} sx={{p: 1}}>
                    <Stack direction='row' justifyContent='space-between' alignItems='center' spacing={2}>
                        ...
                        <Stack direction='row' alignItems='center'>
                            ...
                            <Tooltip title='Modifier le status'>
                                <IconButton data-testid={`status${prospect.id}`} aria-describedby={id} component='span'
                                            onClick={openPopover}>
                                    <MoreVert fontSize='small'/>
                                </IconButton>
                            </Tooltip>
                            <Tooltip title='Modifier le prospect'>
                                <IconButton data-testid={`edit${prospect.id}`} aria-describedby={id} component='span'
                            ...
                            >
                                <Box sx={{m: 2}}>
                                    <FormControl>
                                        <RadioGroup defaultValue={prospect.status} name='status'
                                                    onChange={handleSubmit(changeStatus)}>
                                            <FormControlLabel value='TO_CONTACT' control={<Radio size='small'/>}
                                                              label='À contacter'/>
                                            <FormControlLabel value='CONTACTED' control={<Radio size='small'/>}
                                                              label='Contacté'/>
                                            <FormControlLabel value='CONVERTED' control={<Radio size='small'/>}
                                                              label='Converti'/>
                                        </RadioGroup>
                                    </FormControl>
                                </Box>
                    ...
                    <Box sx={{color: '#4d4d4d'}}>
                        ...
                        <CardViewField icon={<Update/>}
                                       value={parseRatingLastEvaluation(prospect?.rating?.lastEvaluation)}/>
                    </Box>
                    {/* isOpen &&  <ChangeStautsModal prospect={prospect} saveOrUpdateProspectSubmit={saveOrUpdateProspectSubmit}/> */}
                    {/* isOpen &&  <UpdateModal prospect={prospect} saveOrUpdateProspectSubmit={saveOrUpdateProspectSubmit}/> */}
                </Paper>
            </form>
        </FormProvider>
    );
