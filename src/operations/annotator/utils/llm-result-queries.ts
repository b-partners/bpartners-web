...
export const useLlmResultQuery = (roofAnnotatorProperties: Properties) => {
  ...
  const queryFn = async () => {
    const result = await fetch(
      `${baseUrl}?revetement=${annotationCoveringMapper.fromAnalyseResultToDomain(revetement_1).value}&humidit%C3%A9=${humidite_rate}&usure=${usure_rate}&moisissure=${moisissure_rate}&surfaceEnM2=${roof_area_in_m2}&x-api-key=${apiKey}`
    );
